...
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub(crate) unsafe extern "stdcall" fn DispatchMessageW(msg: *const Msg) -> isize {
    unsafe {
        common::dispatch_message(msg, |proc_fn, hwnd, m, wp, lp| {
            let f: extern "stdcall" fn(usize, u32, usize, isize) -> isize =
                std::mem::transmute(proc_fn);
...
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub(crate) unsafe extern "stdcall" fn PostQuitMessage(exit_code: i32) {
    common::post_quit_message(exit_code);
}

/// Posts a message to the message queue of the specified thread and returns without waiting for
...
///
/// # Safety
/// The caller must ensure that `hwnd` is a valid window handle that belongs to the calling thread and that the
/// window procedure associated with the window is designed to handle the posted message to avoid unresponsive behavior or deadlocks.
/// The caller should ensure that the message loop is designed to handle any messages generated by the window
/// procedure in response to the posted message to avoid unresponsive behavior or deadlocks.
/// The caller should ensure that the message loop is designed to handle any messages that may be generated by
/// the window procedure in response to the posted message (for example, by calling `GetMessage` or `PeekMessage`
/// in a loop) to avoid unresponsive behavior or deadlocks.
///
/// # Returns
/// `1` if the function succeeds and the message is posted to the thread's message queue.
/// `0` if the function fails (for example, if `hwnd` is an invalid window handle or if the thread does not have a message queue).
///
/// # Notes
/// This implementation does not set the last error code (GetLastError) on failure and simply returns `0` if the
/// message cannot be posted to the thread's message queue.
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub(crate) unsafe extern "stdcall" fn PostMessageA(
    hwnd: usize,
    msg: u32,
    w_param: usize,
    l_param: isize,
...
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub(crate) unsafe extern "stdcall" fn PostMessageW(
    hwnd: usize,
    msg: u32,
    w_param: usize,
    l_param: isize,
...
///
/// # Notes
/// This implementation does not set the last error code (GetLastError) on failure and simply returns `0` if the message
/// cannot be posted to the thread's message queue.
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub(crate) unsafe extern "stdcall" fn SendMessageA(
    hwnd: usize,
    msg: u32,
    w_param: usize,
    l_param: isize,
...
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub(crate) unsafe extern "stdcall" fn SendMessageW(
    hwnd: usize,
    msg: u32,
    w_param: usize,
    l_param: isize,
...
///
/// # Safety
/// The caller must ensure that `hwnd` is a valid window handle and that the message being processed is appropriate
/// for the window procedure associated with `hwnd` to avoid unresponsive behavior or deadlocks.
/// The caller should ensure that the message loop is designed to handle any messages generated by the default window procedure
/// in response to the message being processed to avoid unresponsive behavior or deadlocks.
///
/// # Returns
/// The return value is the result of the message processing and depends on the message being processed.
/// The return value is typically determined by the default window procedure's handling of the message and may vary based on
/// the specific message and the implementation of the default window procedure.
///
/// # Notes
/// This function is typically called by window procedures to provide default processing for messages that the application does
/// not process. The default window procedure provides standard behavior for many messages, such as moving and resizing windows,
/// handling keyboard input, and managing the window's system menu. The caller should refer to the documentation for the
/// specific messages being processed and the expected behavior from the default window procedure to understand the meaning of
/// the return value from this function.
/// Missing implementation features:
/// - This is a stub that does not perform default processing for system and non-client messages.
/// - Message-specific return semantics are not implemented; the function always returns `0`.
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub(crate) unsafe extern "stdcall" fn DefWindowProcA(
    hwnd: usize,
    msg: u32,
    w_param: usize,
    l_param: isize,
...
#[allow(non_snake_case)]
#[unsafe(no_mangle)]
pub(crate) unsafe extern "stdcall" fn DefWindowProcW(
    hwnd: usize,
    msg: u32,
    w_param: usize,
    l_param: isize,
